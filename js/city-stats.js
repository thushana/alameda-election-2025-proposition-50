...
export function calculateCityStats(data) {
    const stats = {};
    ...
    data.features.forEach((feature) => {
        ...
        if (props.votes) {
            if (props.votes.yes)
                stats[normalizedCity].yes += props.votes.yes;
            if (props.votes.no)
                stats[normalizedCity].no += props.votes.no;
            if (props.votes.total)
                stats[normalizedCity].total += props.votes.total;
        }
    });
